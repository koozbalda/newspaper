...
echo '<h2  align="center" style="color: #ff245c">Админка переехала из практики №5. </h2><br/><br/>';
...
<div>
    <ul class="pagination">
        <li class="prev">
            <a href="<?=$url?>/index.php?page=1&amp;per-page=<?=$per_page?>" data-page="1">
                <span>first</span>
            </a>
        </li>
        <li class="prev <?= ($page - 1) > 0 ? '1' : 'disabled' ?>">

            <a href="<?=$url?>/index.php?page=<?= ($page - 1) > 0 ? ($page - 1) : '1   ' ?>&amp;per-page=<?=$per_page?>"
            ...
            if ($i >= 1 && $i <= $last_page) {
                if ($i == $page) {
                    echo '<li class="active">
                        <a href="'.$url.'/index.php?page=' . $i . '&amp;per-page=' . $per_page . '" data-page="' . $i . '">' . $i . '</a>
                      </li>';
                } else {
                    echo '<li>
                        <a href="'.$url.'/index.php?page=' . $i . '&amp;per-page=' . $per_page . '" data-page="' . $i . '">' . $i . '</a>
        ...
        <li class="next <?= ($page + 1) <= $last_page ? '' : 'disabled' ?>">
            <a href="<?=$url?>/index.php?page=<?= ($page + 1) <= $last_page ? $page + 1 : $last_page ?>&amp;per-page=<?=$per_page?>"
               data-page="<?=$page+1?>">
                &raquo
            </a>
        </li>
        <li class="next">
            <a href="<?=$url?>/index.php?page=<?= !empty($last_page) ? $last_page : 1 ?>&amp;per-page=<?=$per_page?>"
               data-page="<?=$last_page?>">
                last
            </a>
        </li>
